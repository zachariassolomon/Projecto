...
<%
siaadao.Interacao[] interacaos = siaadao.InteracaoDAO.listInteracaoByQuery(null, null);
for(int i = 0; i < interacaos.length; i++) {
		...
		siaadao.Tarefa tarefa = interacaos[i].getTarefa();
		if(tarefa != null) {
			out.print("<td>");
...
out.println("<td></td>");
		...
		siaadao.User user = interacaos[i].getUser();
		if(user != null) {
			out.print("<td>");
