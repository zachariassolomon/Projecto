...
public class ProjetoProcessor {
	...
	public String process() {
		...
		if (action.equals("search")) {
			try {
				TrabAASI.Projeto _projeto = TrabAASI.ProjetoDAO.loadProjetoByORMID(getID());
				if (_projeto!= null) {
			...
			try {
				TrabAASI.Projeto _projeto = TrabAASI.ProjetoDAO.createProjeto();
				copyToBean(_projeto);
				if (TrabAASI.ProjetoDAO.save(_projeto)) {
			...
			try {
				TrabAASI.Projeto _projeto = TrabAASI.ProjetoDAO.loadProjetoByORMID(getID());
				if (_projeto != null && TrabAASI.ProjetoDAO.deleteAndDissociate(_projeto)) {
	...
	private void copyFromBean(TrabAASI.Projeto _projeto) {
	...
	private void copyToBean(TrabAASI.Projeto _projeto) {
